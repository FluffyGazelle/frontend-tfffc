...
const UpdateEvent = () =>{
    ...
    const handleSubmit = async (e) => {
        ...
        const response = await fetch('https://backend-tfffc.herokuapp.com/api/event/addevent', {
            method: 'POST',
    ...
    */
    const handleSubmit = async (e) => {
        ...
        const stuevent = {eventtitle,eventlocation,eventbody,eventquota}
        console.log("in form: ", stuevent)
        const response = await fetch('https://backend-tfffc.herokuapp.com/api/event/getevents/'+ eventData._id, {
            method: 'PATCH',
            body: JSON.stringify(stuevent),
            headers: {
                'Content-Type': 'application/json',
